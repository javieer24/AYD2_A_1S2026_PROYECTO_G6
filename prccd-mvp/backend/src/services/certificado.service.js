...
async function emitirCertificado(sesion_id) {
  ...
  const [existente] = await sequelize.query(
    `SELECT * FROM certificados WHERE sesion_examen_id = :sesion_id`,
    { replacements: { sesion_id }, type: QueryTypes.SELECT }
  );
  ...
  const [ultimo] = await sequelize.query(
    `SELECT numero_bloque, hash_bloque FROM certificados ORDER BY numero_bloque DESC LIMIT 1`,
...
async function verificarCertificado(identificador) {
  const [cert] = await sequelize.query(
    `SELECT * FROM certificados WHERE id_certificado = :id OR hash_documento = :id OR hash_bloque = :id LIMIT 1`,
...
async function verificarCadena() {
  const bloques = await sequelize.query(
    `SELECT * FROM certificados ORDER BY numero_bloque ASC`,
    { type: QueryTypes.SELECT }
  );
