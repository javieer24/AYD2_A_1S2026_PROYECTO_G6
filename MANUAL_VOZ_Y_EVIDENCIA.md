...
## Estado actual del frontend — LO QUE FALTA IMPLEMENTAR

> Revisado el 2026-06-26 sobre `ExamenPage.jsx`.

| Funcionalidad              | Estado en el frontend                                      |
|----------------------------|------------------------------------------------------------|
| Botón "Grabar voz"         | Existe en el UI pero **sin `onClick` ni lógica alguna**    |
| Grabación de pantalla      | **No existe** en ningún archivo del frontend               |
| `getUserMedia` (micrófono) | **Nunca se llama** — el navegador no pedirá permiso        |
| `getDisplayMedia` (pantalla)| **Nunca se llama** — el navegador no pedirá permiso       |

### El navegador SÍ pedirá permisos — cuando se implementen

El navegador muestra un popup de permiso la primera vez que se llama a estas APIs. No es configurable desde el backend, es comportamiento nativo del browser.

- **Micrófono** → se pide al llamar `navigator.mediaDevices.getUserMedia({ audio: true })`
- **Pantalla** → se pide al llamar `navigator.mediaDevices.getDisplayMedia({ video: true })`

Si el usuario **deniega**, la promesa lanza un error `NotAllowedError` y hay que atraparlo.

### Implementación mínima que le falta al botón de voz

El botón en `ExamenPage.jsx` (línea 308) solo tiene el SVG y el texto. Necesita esto:

```js
// Estado que va en el componente
const [grabando, setGrabando] = useState(false);
const mediaRecorderRef = useRef(null);
const chunksRef = useRef([]);

async function grabarYResponder() {
  if (grabando) {
    // Detener grabación
  ...
  try {
    ...
    // ↑ AQUÍ el navegador pide permiso de micrófono
  } catch (err) {
    if (err.name === 'NotAllowedError') {
      setError('Debes permitir el acceso al micrófono para responder por voz.');
    } else {
  ...
  recorder.onstop = async () => {
    ...
    formData.append('pregunta_id', String(preguntaActual.id));

    setCargando(true);
    try {
      const res = await fetch('http://localhost/api/voz/responder', {
        method: 'POST',
      ...
      if (!res.ok) {
        setError(data.message || 'Error al procesar la respuesta de voz');
        return;
      }
      // Continuar igual que al responder con botón
      setHistorial(prev => [...prev, {
        numero: numeroPregunta,
        nivel: preguntaActual.nivel,
        respuesta: data.respuesta_enviada,
        correcto: data.resultado_examen.correcto,
      }]);
      if (data.resultado_examen.terminado) {
        setDictamen(data.resultado_examen.dictamen);
        setFase('resultado');
      } else {
        setPreguntaActual(data.resultado_examen.pregunta);
        setNumeroPregunta(data.resultado_examen.numero);
        setSeleccion('');
      }
    } catch {
      setError('Error de conexión al enviar la respuesta de voz');
    } finally {
      setCargando(false);
    }
  };
...
Y el botón queda así:

```jsx
<button
  type="button"
  onClick={grabarYResponder}
  className={`flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-black text-white shadow-md ${
    grabando ? 'bg-red-600' : 'bg-indigo-700'
  }`}
>
  {grabando ? 'Detener grabación' : 'Grabar respuesta por voz'}
</button>
```

### Implementación mínima de grabación de pantalla

Esto va aparte del examen — se inicia al comenzar la sesión y se sube al terminar:
...
async function iniciarGrabacionPantalla(sesionId, token) {
  try {
    screenStream = await navigator.mediaDevices.getDisplayMedia({ video: true });
    // ↑ AQUÍ el navegador pide permiso de pantalla (no se puede omitir)
  } catch (err) {
    if (err.name === 'NotAllowedError') {
      console.warn('El candidato denegó compartir pantalla');
    }
    return;
  }
  ...
  screenRecorder.onstop = async () => {
    ...
    const formData = new FormData();
    formData.append('archivo', videoBlob, 'grabacion_pantalla.webm');
    formData.append('sesion_id', String(sesionId));
    formData.append('tipo', 'VIDEO');

    await fetch('http://localhost/api/telemetria/evidencia', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: formData,
    });
...
// Llamar esto al terminar o suspender el examen:
function detenerGrabacionPantalla() {
  if (screenRecorder && screenRecorder.state !== 'inactive') {
    screenRecorder.stop(); // dispara onstop que sube el video
  }
}
```
